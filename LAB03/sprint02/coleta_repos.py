...
TARGET_REPOS = 200
PAGE_SIZE = 100  # máximo permitido pela API do GitHub (era 10 na sprint01)
MIN_PRS = 100
OUTPUT_CSV = Path(__file__).resolve().parent / "repositorios.csv"

CSV_FIELDS = ["name_with_owner", "stars", "primary_language", "prs_merged", "prs_closed", "prs_total"]
...
def graphql_request(query: str, variables: dict) -> dict:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "lab03-s02-coleta-repos",
        },
    )
...
def main() -> int:
    ...
    while len(rows) < TARGET_REPOS:
        variables = {"pageSize": PAGE_SIZE, "cursor": cursor, "starsFilter": stars_filter}
